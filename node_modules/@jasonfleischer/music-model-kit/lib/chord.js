class Chord {
  ...
  getNoteArray(all_notes, range) {
    ...
    function isNoteWithinRange(sequence_number){
      return range.min >= sequence_number && range.max <= sequence_number;
    }

    var note_array = [];
    var i;
    for(i=0; i<note_sequence.length; i++){
      let sequence_number = this.root_note.note_value + this.note_sequence[i];
      if(isNoteWithinRange(sequence_number)){
        note_array.push[all_notes[sequence_number]];
      }
    }
    return note_array;
  }
