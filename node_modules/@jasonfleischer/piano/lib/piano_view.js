...
class PianoView {
	constructor(id = "piano_view_id", width = 1000, range = musicKit.piano_range, onClick, onHover) {
		...
		if(onHover !== undefined) {
			this.addHoverEventListeners(onHover);
		}
	}

	buildCanvases(onHover) {
	...
	addClickEventListeners(onClick) {
		...
		pianoView.addEventListener("click", function(event){
		    ...
		    let position =	this.getPosition(pianoView);
	...
	addHoverEventListeners(onHover) {
		...
		pianoView.addEventListener("mousemove", function(event){
		    ...
		    let position =	getPosition(pianoView);
		    let x = (event.clientX - position.x) * (WIDTH/width);
		    let y = (event.clientY- position.y) * (WIDTH/width);
		    ...
		    if(foundKey === undefined){
		    	...
		    	if(previousKey.note.midi_value === foundKey.note.midi_value) {
		    		...
		    		onHover(previousKey.note, false);
		    		previousKey = foundKey;
		    		onHover(foundKey.note, true);
		    	}		    	
		    }
		});
