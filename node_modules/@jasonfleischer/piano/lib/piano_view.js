...
class PianoView {
	constructor(id) {
		...
		this.min_note_value = 21;
		this.max_note_value = 108;
	...
	resize(newWidth) {
		...
		document.getElementById(this.is).style.height = newHeight + "px";
	...
	draw_background(){
		...
		for(i = this.min_note_value; i <= this.max_note_value; i++){
			...
			if(!note.note_name.is_sharp_or_flat){
				...
				let key = new PianoKey(x, this.BORDER_WIDTH, 
		...
		for(k = this.min_note_value; k <= this.max_note_value; k++){
			...
			if(note.note_name.is_sharp_or_flat){
				...
				if( flat_key != undefined && sharp_key != undefined) {
					...
					let key = new PianoKey(x, this.BORDER_WIDTH, black_key_width, black_key_height, note, "#333")
					black_keys.push(key)
					this.note_value_to_piano_key_map[note.note_value] = key
				}
			}
