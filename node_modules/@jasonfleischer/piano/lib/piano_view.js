...
class PianoView {
	...
	buildCanvases() {
	    ...
	    this.canvas_background.id = "piano_background_canvas_"+this.id;
	    this.canvas_background.style.position = "absolute"
	    this.canvas_background.style.left = "0px"
	    this.canvas_background.style.right = "0px"
	    this.canvas_background.style.width = width + "px";
		this.canvas_background.style.height = height + "px";
		this.canvas_background.width = width;
		this.canvas_background.height = height;
	    pianoView.appendChild(this.canvas_background);

	    this.white_keys_canvas = document.createElement('canvas'); 
	    this.white_keys_canvas.id = "piano_white_keys_canvas_"+this.id;
	    this.white_keys_canvas.style.position = "absolute"
	    this.white_keys_canvas.style.left = "0px"
	    this.white_keys_canvas.style.right = "0px"
	    this.white_keys_canvas.style.width = width + "px";
		this.white_keys_canvas.style.height = height + "px";
		this.white_keys_canvas.width = width;
		this.white_keys_canvas.height = height;
	    pianoView.appendChild(this.white_keys_canvas);

	    this.black_keys_canvas = document.createElement('canvas'); 
	    this.black_keys_canvas.id = "piano_black_keys_canvas_"+this.id;
	    this.black_keys_canvas.style.position = "absolute"
	    this.black_keys_canvas.style.left = "0px"
	    this.black_keys_canvas.style.right = "0px"
	    this.black_keys_canvas.style.width = width + "px";
		this.black_keys_canvas.style.height = height + "px";
		this.black_keys_canvas.width = width;
		this.black_keys_canvas.height = height;
	    pianoView.appendChild(this.black_keys_canvas);
	...
	resize(newWidth) {
		...
		var newHeight = this.white_key_height;// * (this.HEIGHT/newWidth);
		...
		/*this.black_keys_canvas.width = newWidth;
		this.black_keys_canvas.height = newHeight;

		this.white_keys_canvas.width = newWidth;
		this.white_keys_canvas.height = newHeight;

		this.canvas_background.width = newWidth;
		this.canvas_background.height = newHeight;*/
	}
	...
	draw_background(){
		...
		let white_key_width = Math.floor((this.WIDTH - ((this.number_of_white_keys+1)*this.BORDER_WIDTH) )/ number_of_white_keys);
		this.white_key_height =  Math.floor(white_key_width * 5);

		log.e('jj '+this.white_key_height + ' '+this.number_of_white_keys);

		var white_keys = [];
		
		var i;
		var x = this.BORDER_WIDTH + (this.WIDTH - ((white_key_width + this.BORDER_WIDTH ) * this.number_of_white_keys))/2;//.WIDTH - ((number_of_white_keys+1)*this.BORDER_WIDTH) )/2;
		for(i = this.min_note_value; i <= this.max_note_value; i++){
			var note = musicKit.all_notes[i];
			if(!note.note_name.is_sharp_or_flat){
